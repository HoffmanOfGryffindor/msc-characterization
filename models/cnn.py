...
class CNNModel(keras.Model):
    def __init__(self, config, shape):
        ...
        if self.regression:
            ...
            self.final_layer_size, self.final_layer_activation = (
                self.num_classes,
                "softmax",
            )
        self.model = keras.Sequential(
            [
                keras.layers.Conv2D(
                    32, self.filter_dim, activation="relu", input_shape=(self.data_dim)
                ),
                ...
                keras.layers.Dense(
                    self.final_layer_size, activation=self.final_layer_activation
                ),
            ]
        )
...
class MobileNet(keras.Model):
    def __init__(self, config: dict, shape: list):
        ...
        self.mobile.add(
            keras.layers.Dense(config["data"]["num_classes"], activation="softmax")
        )
